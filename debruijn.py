...
class DeBruijnGraph (MyGraph):
    ...
    def add_edge(self, o, d):
        if o not in self.graph.keys(): self.add_vertex(o)
        if d not in self.graph.keys(): self.add_vertex(d)
    ...
    def create_deBruijn_graph(self, frags):  # para cada um dos frags calculamos o seu sufixo e prefixo para colocar nos vertex e depois acrescentamo-los aos edges.
        for seq in frags:
            suf = suffix(seq)
            self.add_vertex(suf)
            pref = prefix(seq)
